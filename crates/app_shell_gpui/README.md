...
This crate is the first `gpui-ce` app-shell scaffold for `Fronda`.

It is intentionally split in two layers:

- the library target keeps shell copy and future shell state in plain Rust so workspace tests stay portable
...
Launch the placeholder desktop shell on a supported platform:
