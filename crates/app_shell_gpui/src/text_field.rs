//! Reusable single-line text field with real platform text input — IME
//! composition (CJK), cursor movement, selection, and clipboard — built on
//! gpui's `EntityInputHandler` (ported from the upstream gpui input example).
//!
//! Views embed an `Entity<TextField>` and subscribe to [`TextFieldEvent`].
//! Call [`bind_text_field_keys`] once at app boot; bindings are scoped to
//! the `FrondaTextField` key context so they never affect global shortcuts.
//!
//! Word boundaries use `char` boundaries (not full grapheme clusters) to
//! avoid a new dependency; multi-scalar emoji take extra arrow presses.
...
/// Register the field's key bindings. Idempotent per app; call at boot.
pub fn bind_text_field_keys(cx: &mut App) {
    const CTX: Option<&str> = Some("FrondaTextField");
    cx.bind_keys([
...
pub enum TextFieldEvent {
    /// Enter was pressed.
    Submitted,
    /// The content changed (typing, paste, IME commit).
    Edited,
}

/// Single-line editable text field entity.
...
impl EntityInputHandler for TextField {
    ...
    fn replace_and_mark_text_in_range(
        ...
        // START of the replaced region (the upstream example adds range.end
        // to the end — wrong whenever the previous marked range was
        // non-empty; caret lands mid-char during CJK composition).
...
/// Custom element: shapes the line, paints selection/cursor/marked underline,
/// and registers the platform input handler during paint.
