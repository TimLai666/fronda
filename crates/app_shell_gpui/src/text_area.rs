...
impl TextArea {
    ...
    /// Same swallow rule as `TextField::handle_key_down`: characters arrive
    /// via the platform text-input path, so stop unbound modifier-free keys
    /// from bubbling into global shortcuts. Escape and Tab are left to the
    /// host view (cancel / field navigation). Enter/up/down are bound actions
    /// here, so they only reach this handler with a modifier held.
    fn handle_key_down(
        &mut self,
        event: &KeyDownEvent,
        _window: &mut Window,
