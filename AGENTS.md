...
- `render_core::compositor` is the pure CPU frame compositor: `compose_frame` flattens a timeline frame into RGBA (layer order, per-clip transform/crop/opacity, rotation, all 16 blend modes via `blend_rgb`, keyframe-resolved via `timeline_core::resolved_*_at`, bilinear sampling). Also composites shape annotations (`rasterize_shape`: rect/oval/circle fill+stroke; arrow/line pending), applies chroma key (`apply_chroma_key`, PR #8) and single-parameter colour adjustments (`apply_color_adjustments`: exposure/contrast/saturation/brightness). Text overlays + grading/blur/vignette effects are the remaining passes. `render_sequence` drives per-frame compose for the exporter. Media decode is a `fetch_source` closure (platform adapter), so the module is fully unit-tested with synthetic sources.
