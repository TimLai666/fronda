...
- COMPOUND CLIPS (#155, `timeline_core::compound`): `create_compound_clip` groups adjacent same-track clips into one compound `Clip` (`compound_timeline_id = Some(id)`) whose constituents move into a nested `Timeline` (re-based to 0) under `Timeline::compound_timelines[id]`; `dissolve_compound_clip` flattens it back. Nothing renders `compound_timeline_id` directly — every render/export path calls `flatten_compound_clips` first (in `compose_frame`, `mix_timeline_audio`, XMEML + FCPXML export; O(1) borrow fast-path when `compound_timelines.is_empty()`). Do NOT add compound-clip handling to individual decode closures — they key on the manifest, so flattened constituents already decode. v1 is single-track; a transform/fades/**trim/speed** applied to the compound clip *itself* aren't yet composed onto the group (`flatten`/`dissolve` offset only by `start_frame`), so avoid trimming/retiming a compound clip until that's handled — group→render→dissolve with no manual edit of the compound is exact (tested). It has no persistent display name (no Clip/Timeline name field; `MediaSource` is file-only).
