...
| #193 | FCPXML export v1 baseline                | DONE (v1)   | render_core (fcpxml_export.rs) + ExportMode::Fcpxml + write_interchange (save dialog writes .xml/.fcpxml); refinements #197/#254 + #247 relink-by-filename pending (#214/#206/#247-source-timecode done) |
| #247 | FCPXML source timecode                   | DONE        | render_core fcpxml_export.rs start_timecode_frames — asset `start` = embedded timecode, asset-clip in-point += origin (round(tc_frame/quanta*fps)); relink-by-filename still pending |
