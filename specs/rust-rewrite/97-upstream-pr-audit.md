...
| #65 | Font weight in TextStyle | **DIVERGES (on-disk compat bug)** | core_model (TextStyle.font_weight) | Rust→Rust serde round-trips, but DIVERGES from current Swift on-disk format (9a3ae50): Swift `TextStyle` has `isBold: Bool = true` + `isItalic: Bool = false` in its `CodingKeys` (inferring bold from fontName traits when the key is absent) and NO `fontWeight`; Rust has `font_weight: f64` (default 400) and reads neither `isBold`/`isItalic` nor infers from fontName. Bidirectional data loss: a Swift-authored `.palmier` loads bold/italic text as regular (weight 400) in Rust, and Rust's `fontWeight` is dropped by Swift. Default also diverges (Swift bold-by-default). **Fix (careful, high-stakes — touches the on-disk format):** add a serde bridge (`#[serde(from/into)]` wire struct) mapping `isBold`/`isItalic` ↔ `font_weight` (with fontName bold/italic inference for keyless files) and writing BOTH so each side reads its own keys; add an `is_italic` field (ripples to every `TextStyle {}` literal + Default across crates); the FCPXML title `fontFace` + the text renderer then reflect Swift's bold/italic. Surfaced 2026-07-05 by adversarial FCPXML title verification (the face was derived from `font_weight`, correct for Rust-authored data but wrong for Swift-authored). |
