...
| #207 | ripple_delete_ranges: per-call sync-lock exemption (`ignoreSyncLockedTracks`) | `timeline_core/src/workflow.rs` RippleDeleteConfig | M | **NOT STARTED.** Add `ignore_sync_lock_track_indices` to `RippleDeleteConfig`; exempt those tracks from BOTH the sync-locked clear-set AND the executor shift (leave them in place). CRITICAL: an ignored-yet-sync-locked track is shifted-without-cut, so it CAN collide — the shift refuse validation (removed in #227 as dead code for the empty-ignore case) MUST be reintroduced for the ignored-track path only. |
