...
- [x] `EDT-008`: The seams between the Agent/Media/Inspector columns and the seam above the Toolbar+Timeline region are draggable dividers (pure `pane_resize::clamp_resize`). Clamps: agent 240–640, media ≥ 280 + tab-rail width, inspector ≥ 240 (was 150 pre-#327; Swift inspectorMin = EditorPanel.minimumWidth), timeline height 100–700, and no drag reduces the Preview column below 400 wide (or the Preview region below 320 tall). The pane's own minimum wins over the space guard on tiny windows.
